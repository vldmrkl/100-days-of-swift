...
class ViewController: UITableViewController {
        ...
        if isPossible(word: answerLowercased) {
            if isOriginal(word: answerLowercased) {
                if isReal(word: answerLowercased) {
                    ...
                    showErrorMessage(title: "Word not recognised", message: "You can't just make up words!")
                }
            } else {
                showErrorMessage(title: "Word used already", message: "C'mon! Be original.")
            }
        } else {
    ...
    func isReal(word: String) -> Bool {
        ...
        let tooShort = word.count <= 3
        let sameAsTitle = word == title
        
        return misspeledRange.location == NSNotFound && !tooShort && !sameAsTitle
    }
